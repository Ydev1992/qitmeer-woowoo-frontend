...
const Brc20InscConfirmDialog: React.FC<ConfirmPopupProps> = (props) => {
  const { isOpenBuy, setIsOpenBuy, handleBuyOkClick, brc20State } = props;
  ...
  const getPreviewList = () => {
    let prevList = [];
    for (let i = 0; i < brc20State.mintCount; i++) {
      prevList.push(
        <p className="rounded-lg p-[8px] my-1 bg-white bg-opacity-10">{`{"p": "brc-20", "op": "${brc20State.op}", "tick": "${brc20State.tick}", "amt": "${brc20State.mintAmount}"}`}</p>
      );
    }
    return prevList;
  };
...
export default Brc20InscConfirmDialog;
