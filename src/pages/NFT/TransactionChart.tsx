...
const Loading = () => {
  ...
  return (
    <div
      className={`flex ${
        innerHeight < 725 ? "h-[200px]" : "h-[220px]"
      } w-full items-center justify-center pt-10`}
    >
...
const TransnactionChart = ({
  ...
  const totalPrice = pricehistory.length ? pricehistory[pricehistory.length - 1] : 0;
  const chartData: any = {
    series: [
      {
        name: "Price",
  ...
  const innerHeight = window && window.innerHeight ? window.innerHeight : 0;
  const periodTexts: any = { 0: "24hrs", 1: "1W", 2: "1M", 3: "1Y", 4: "ALL" };
...
export default TransnactionChart;
