...
export default function NFTPanel({ selectedCategories }: { selectedCategories: any }) {
  ...
  const wrappedListings = listings.filter(
    (listing: any) =>
      listing.endTimestamp >= Date.now() / 1000 && listing.startTimestamp <= Date.now() / 1000
  );

  const wrappedAuctions = auctions.filter(
    (auction: any) =>
      auction.endTimestamp >= Date.now() / 1000 && auction.startTimestamp <= Date.now() / 1000
  );
  ...
  return (
    <div>
      <div className="text-2xl text-[#C4C4C4]">{numberWithCommas(wrappedItems.length)} Result</div>
